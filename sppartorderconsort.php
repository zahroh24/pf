...
<a button type="button" style="width: 150px; margin-bottom: 1rem" class="btn btn-outline-danger" href="../pf/sppartordercon.php?user=<?php echo $user;?>&level=<?php echo $level;?>">← Back to All Date</button></a>
<br/>
...
<form>
        ...
        <tbody>
          <?php
          $spareque = mysqli_query($connect2,"SELECT * FROM sparepartorder WHERE month(Orderdate)='$datecon' ORDER BY ID DESC");
...
<form>
        ...
        <tbody>
          <?php
          $spareque = mysqli_query($connect2,"SELECT * FROM sparepartorder WHERE Status='order confirmation' and month(Orderdate)='$datecon' ORDER BY ID desc");
          while ($sparefetch = mysqli_fetch_array($spareque)){
          $dietype = $sparefetch['dietype'];
          $die_name = $sparefetch['die_name'];
